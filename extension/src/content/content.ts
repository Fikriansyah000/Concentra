 ...
 * 100% Self-Contained IIFE.
 * Displays floating HUD on any webpage while background offscreen AI tracks focus & distractions.
 */
(function() {
  'use strict';
  ...
  function createHUD() {
    ...
    hudContainer.innerHTML = `
      <style>
        ...
        .chud-dir {
          font-size: 10px !important; font-weight: 600 !important; color: #94a3b8 !important; text-align: right !important;
        }
        .chud-status-row {
          display: flex !important; align-items: center !important; gap: 6px !important;
        ...
        .chud-pulse { animation: chud-pulse 1.5s ease-in-out infinite !important; }
      </style>
      ...
      <div id="chud-card" class="chud-card">
        <div class="chud-header">
          <div class="chud-logo">
            ...
            <span class="chud-title">Concentra AI Monitor</span>
          </div>
        ...
        <div class="chud-status-row">
          ...
          <span id="chud-status-label" class="chud-status-label">AI memantau fokus Anda</span>
        </div>
        ...
        <div class="chud-score-row">
          ...
          <span id="chud-dir-text" class="chud-dir">Fokus ke Layar</span>
        </div>
      </div>

      <div id="chud-mini-pill" class="chud-mini-pill">
        <span id="chud-mini-dot" style="width:8px !important;height:8px !important;border-radius:50% !important;background:#10b981 !important;"></span>
  ...
  function updateHUD(score: number, isDistracted: boolean, headDirection: string, isEyesClosed?: boolean) {
    if (!hudContainer) return;

    if (hudScoreText) hudScoreText.textContent = `${score}%`;

    const miniScore = document.getElementById('chud-mini-score');
    const miniDot = document.getElementById('chud-mini-dot');
    const statusLabel = document.getElementById('chud-status-label');

    if (miniScore) miniScore.textContent = `${score}% Fokus`;

    if (hudScoreBadge) {
      if (isEyesClosed) {
        hudScoreBadge.style.background = 'rgba(239,68,68,0.2)';
        hudScoreBadge.style.color = '#f87171';
        hudScoreBadge.style.borderColor = 'rgba(239,68,68,0.4)';
        if (hudStatusDot) hudStatusDot.style.background = '#ef4444';
        if (miniDot) miniDot.style.background = '#ef4444';
        if (statusLabel) statusLabel.textContent = 'Mata Terpejam (Merem)';
      } else if (score >= 80) {
        ...
        if (miniDot) miniDot.style.background = '#10b981';
        if (statusLabel) statusLabel.textContent = 'Konsentrasi Optimal';
      } else if (score >= 50) {
        ...
        if (miniDot) miniDot.style.background = '#f59e0b';
        if (statusLabel) statusLabel.textContent = 'Fokus Sedang';
      } else {
        ...
        if (hudStatusDot) hudStatusDot.style.background = '#ef4444';
        if (miniDot) miniDot.style.background = '#ef4444';
        if (statusLabel) statusLabel.textContent = 'Terdeteksi Distraksi';
      }
    }

    if (hudDirectionText) {
      if (isEyesClosed) {
        hudDirectionText.textContent = '😴 Merem';
        hudDirectionText.style.color = '#f87171';
      } else {
        const labels: Record<string, string> = {
          front: 'Fokus ke Layar',
          left: 'Menoleh Kiri',
          right: 'Menoleh Kanan',
          down: 'Menunduk',
          up: 'Mendongak',
        ...
        case 'LIVE_FOCUS_METRIC':
          ...
          if (message.payload) {
            updateHUD(
              ...
              message.payload.isEyesClosed || false
            );
          }
          sendResponse({ success: true });
          break;
